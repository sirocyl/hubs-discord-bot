...
async function establishBridging(hubState, bridges) {
  ...
  presenceRollups.on('new', ({ kind, users, fresh }) => {
    try {
      ...
      console.error(ts(`Error relaying presence ${kind} in ${hubState.id}.`));
    }
  });
  presenceRollups.on('update', ({ kind, users, fresh }) => {
    try {
      ...
      console.error(ts(`Error updrelaying presence update ${kind} in ${hubState.id}.`));
    }
  });
  ...
  reticulumCh.on('rescene', (timestamp, id, whom, scene) => {
    try {
      ...
      console.error(ts(`Error relaying presence scene change by ${whom} (${id}) in ${hubState.id}.`));
    }
  });
  reticulumCh.on('renamehub', (timestamp, id, whom, name, slug) => {
    try {
      ...
      console.error(ts(`Error relaying name change by ${whom} (${id}) in ${hubState.id}.`));
    }
  });
  ...
  reticulumCh.on("message", (timestamp, id, whom, type, body) => {
    try {
      ...
      console.error(ts(`Error relaying message of type ${type} from ${whom} (${id}) via ${hubState.id}.`));
    }
  });
  ...
  reticulumCh.on('sync', async () => {
    try {
      ...
      console.error(ts(`Error updating channel presence icons in ${hubState.id}`));
    }
  });
